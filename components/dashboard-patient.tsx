...
export function DashboardPatient() {
  ...
  const [lastVisitSummary, setLastVisitSummary] = useState<LastVisitSummary | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (patientError || !patientProfile) throw new Error("Could not find a patient profile for the current user.");
        ...
        const [
            ...
            supabase.from('ehr')
                .select('created_at, doctors(full_name), diagnosis(diagnosis_description)')
                .eq('patient_id', patientId)
                .order('created_at', { ascending: false })
                .limit(1)
                .single(),
            // Query 3: Get total visit count
            supabase.from('ehr').select('*', { count: 'exact', head: true }).eq('patient_id', patientId)
        ]);
        ...
        if (latestQueueRes.data) {
            ...
            setLatestQueue({
                ...
                queueDate: queueDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }),
                queueTime: queueDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }),
            });
        }
        ...
        if (lastEhrRes.data) {
            ...
            const doctor = Array.isArray(ehr.doctors) ? ehr.doctors[0] : ehr.doctors;
            const diagnosis = Array.isArray(ehr.diagnosis) ? ehr.diagnosis[0] : ehr.diagnosis;

            setLastVisitSummary({
                name: patientName,
                totalVisits: totalVisitsRes.count ?? 0,
                lastDiagnosis: diagnosis?.diagnosis_description ?? 'N/A',
                lastDoctor: doctor?.full_name ?? 'N/A',
                lastVisitDate: visitDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })
            });
        } else {
            ...
            <main className="flex-1 p-4 md:p-6 lg:p-8 space-y-8">
              ...
              <Card className="relative overflow-hidden col-span-full flex flex-col md:flex-row items-center">
                ...
                <div className="relative z-10 p-8 text-white md:w-2/3">
                  <h1 className="text-3xl font-bold">{greeting}, {lastVisitSummary?.name || 'User'}!</h1>
                  <p className="mt-2 text-blue-100">Here’s a summary of your health journey and upcoming appointments.</p>
                </div>
              </Card>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <Card className="border-l-4 border-l-[#3FB6F6] overflow-hidden">
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription>Your most recent queue information</CardDescription>
                  </CardHeader>
                  <CardContent>
                    {latestQueue ? (
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <div className="space-y-1">
                            ...
                            <p className="text-2xl font-bold text-[#3FB6F6]">{latestQueue.queueNumber}</p>
                          </div>
                          <Badge className={getStatusColor(latestQueue.status)}>{latestQueue.status}</Badge>
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm font-medium text-gray-500">Service / Department</p>
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <p>{latestQueue.queueDate}, {latestQueue.queueTime}</p>
                          </div>
                        </div>
                      </div>
                    ) : (
                ...
                <Card className="border-l-4 border-l-[#34D399] overflow-hidden">
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription>Information from your most recent visit</CardDescription>
                  </CardHeader>
                  <CardContent>
                    {lastVisitSummary ? (
                       <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <div className="space-y-1">
                            ...
                            <p className="text-2xl font-bold text-[#34D399]">{lastVisitSummary.totalVisits}</p>
                          </div>
