...
export function MedicalRecordsSystem() {
  const [userRole, setUserRole] = useState<"doctor" | "patient" | "admin">("doctor")
  ...
  // Sample EHR records for selected patient
  ...
  const handlePatientSelect = (patient: Patient) => {
    ...
    const patientEHRs = ehrRecords.filter((ehr) => ehr.patientId === patient.id)
    if (patientEHRs.length > 0) {
      setSelectedEHR(patientEHRs[0]) // Select the most recent active EHR
    }
  ...
  const handleCreateNewEHR = () => {
    toast({
      ...
      description: "A new Electronic Health Record has been created for this patient",
      duration: 3000,
    })
      ...
      <div className="space-y-8">
        <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900">Medical Records System</h1>
            <p className="mt-2 text-gray-600">Search and manage patient electronic health records</p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Search for a patient to view their medical records</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex gap-4">
              <div className="relative flex-1">
                <Search className="absolute left-2 top-2.5 h-4 w-4 text-gray-500" />
                <Input placeholder="Search by name, ID, or phone number..." className="pl-8" />
              </div>
                  ...
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        <Badge className={getStatusColor(patient.insuranceStatus.toLowerCase())}>
                          {patient.insuranceStatus}
                        </Badge>
                        <p className="text-sm text-gray-500 mt-1">
                          {ehrRecords.filter((ehr) => ehr.patientId === patient.id).length} EHR Records
                        </p>
                      </div>
                    </div>
                  </CardContent>
  ...
  if (!selectedPatient || !selectedEHR) {
    return <div>Loading...</div>
  }

  const patientEHRs = ehrRecords.filter((ehr) => ehr.patientId === selectedPatient.id)
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setShowNewEHRDialog(true)}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select an EHR record to view details</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {patientEHRs.map((ehr) => (
              <Card
                key={ehr.id}
                className={`cursor-pointer transition-all ${selectedEHR?.id === ehr.id ? "ring-2 ring-[#3FB6F6] bg-blue-50" : "hover:shadow-md"}`}
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="diagnoses" className="space-y-4 pt-4">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium">Diagnosis History</h3>
                {userRole === "doctor" && (
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="font-medium mt-1">{note.doctor}</p>
                          </div>
                    ...
                    <TableRow key={vaccination.id}>
                      ...
                      <TableCell>
                        {vaccination.nextDueDate ? (
                          <Badge className="bg-yellow-100 text-yellow-800">{vaccination.nextDueDate}</Badge>
                        ) : (
      ...
      <Dialog open={showNewEHRDialog} onOpenChange={setShowNewEHRDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Create New EHR Record</DialogTitle>
            <DialogDescription>Create a new Electronic Health Record for {selectedPatient.name}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select>
                ...
                <SelectContent>
                  <SelectItem value="f1">HealthSync General Hospital</SelectItem>
                  <SelectItem value="f2">City Medical Center</SelectItem>
                  <SelectItem value="f3">Community Health Clinic</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowNewEHRDialog(false)}>
              Cancel
            </Button>
            <Button className="bg-gradient-to-r from-[#3FB6F6] to-[#34D399]" onClick={handleCreateNewEHR}>
              Create EHR
            </Button>
          </DialogFooter>
        </DialogContent>
