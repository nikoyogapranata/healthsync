...
export function DashboardAdmin() {
  ...
  useEffect(() => {
    const getAdminProfile = async () => {
      ...
      if (adminError || !adminProfile?.healthcare_facility_id) {
        setError("Could not find admin profile or facility assignment.");
        setLoading(false);
        return;
      }
  ...
  useEffect(() => {
    const fetchAllDashboardData = async () => {
      ...
      try {
        ...
        // Step 1: Get doctor IDs for the facility
        const { data: facilityDoctors, error: facilityDoctorsError } =
          await supabase
            .from("doctor_healthcare_facility")
            .select("doctor_id")
            .eq("healthcare_facility_id", healthcareFacilityId);

        if (facilityDoctorsError) throw facilityDoctorsError;
        const doctorIds = facilityDoctors.map((fd) => fd.doctor_id);

        // Step 2: Modify your Promise.all to use the new filters
        const [doctorsRes, queueRes, dailyVolumeRes] = await Promise.all([
          // Filter doctors by the IDs you just fetched
          supabase
            .from("doctors")
            .select("doctor_id, specialization, active_status")
            .in("doctor_id", doctorIds),
          // Filter queue data by the facility ID
          supabase
            .from("queue")
            .select("queue_status, created_at, departments ( name )")
            .eq("healthcare_facility_id", healthcareFacilityId)
            .gte("created_at", todayStart.toISOString()),
          // Filter daily volume data by the facility ID
          supabase
            .from("queue")
            .select("created_at")
            .eq("healthcare_facility_id", healthcareFacilityId)
        ...
        if (!doctorsRes.data || !queueRes.data || !dailyVolumeRes.data) {
          throw new Error(
            "One or more data queries returned null. Check RLS policies."
          );
        }
        ...
        setStats({
          ...
          patientsInQueue: todaysQueues.filter(
            (q) => q.queue_status === "Waiting"
          ).length,
          totalDoctors: doctors.length,
          completedToday: todaysQueues.filter(
        ...
        setDoctorSpecializationData(
          Object.entries(specCounts).map(([name, value]) => ({ name, value }))
        );
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Breakdown of patient statuses for today.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      >
                        {doctorStatusData.map((entry, index) => (
                          <Cell
                            ...
                            fill={
                              PIE_COLORS_DOCTORS[
                                index % PIE_COLORS_DOCTORS.length
                              ]
                            }
                          />
                        ))}
            ...
            <div className="grid grid-cols-1 gap-8">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Patient traffic trends over the past week.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart
                      data={dailyQueueVolume}
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Busiest departments based on today's queue.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
